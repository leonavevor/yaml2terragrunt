...
class TestMain(unittest.TestCase):
    ...
    def test_main(self, mock_execute_terragrunt, mock_generate_terragrunt_files, mock_parse_config):
        with patch.object(sys, "argv", ["cli.py", "-f", "test.yaml"]):
            main()
            mock_parse_config.assert_called_once_with("test.yaml")
    ...
    @patch("src.cli.generate_terragrunt_files")
    @patch("src.cli.execute_terragrunt")
    def test_main_no_args(self, mock_execute_terragrunt, mock_generate_terragrunt_files, mock_parse_config):
