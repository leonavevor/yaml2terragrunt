...
[Individual Contributor License Agreement](https://yaml2terragrunt/docs/legal/contributor-agreement.html).
...
```
git clone https://github.com/leonavevor/yaml2terragrunt.git
cd yaml2terragrunt
```
...
Now you should have a fully functional development environment for the Yaml2terragrunt project. You can start making changes, running tests, and contributing to the project.
...
```
docker build -t yaml2terragrunt -f docker/Dockerfile .
```
...
2. Navigate to the `yaml2terragrunt/website` directory.
...
The built documentation will be available in the `yaml2terragrunt/website/_site` directory.

## Coding Standards

### Python Compatibility

Yaml2terragrunt supports Python versions 3.8, 3.9, 3.10, 3.11, and 3.12. When contributing code, ensure compatibility with these supported Python versions.
...
The project uses [pytest](https://docs.pytest.org/en/latest/) for running unit tests. The test files are located in the `yaml2terragrunt/tests` directory and follow the naming convention `test_*.py`.
...
These workflows are triggered on push and pull request events to the `main` branch, ignoring changes to the `yaml2terragrunt/website/**` and `README.md` files.
...
When contributing new features or making changes to existing code, ensure that you write appropriate tests to maintain code coverage. Follow the existing patterns and naming conventions used in the `yaml2terragrunt/tests` directory.

If you need to mock or create test data, consider adding it to the test files or creating separate fixtures or utility functions within the `yaml2terragrunt/tests` directory.
